...
export default [
    {
        ...
        content: `Add LIFF using view ${'--name'.code} and ${'--url'.code} options. We make ${'--type'.code} option as optional in FireLIFF CLI to shorten the command line. So you can omit ${'--type'.code}, it will be ${'full'.code} by default.` + EOL + EOL +
            `fliff add --name ${'<viewName>'.input} --url ${'<viewUrl>'.input}`.code + EOL +
            `fliff add --name ${'<viewName>'.input} --url ${'<viewUrl>'.input} --type ${'<viewType>'.input}`.code
    },
    {
        header: 'Options',
        optionList: [
