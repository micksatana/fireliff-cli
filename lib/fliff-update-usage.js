...
export default [
    {
        ...
        content: `Update using LIFF ID` + EOL + EOL +
            `fliff update --id ${'<liffId>'.input} --type ${'<viewType>'.input} --url ${'<viewUrl>'.input}`.code + EOL + EOL +
            `Update using LIFF name` + EOL + EOL +
            `fliff update --name ${'<viewName>'.input} --type ${'<viewType>'.input} --url ${'<viewUrl>'.input}`.code + EOL + EOL +
            `To update Bluetooth® Low Energy (BLE) flag` + EOL + EOL +
            `fliff update --id ${'<liffId>'.input} --ble ${'<true|false>'.input}`.code
    },
    {
        header: 'Options',
        optionList: [
