...
describe('fliff init success', () => {
  ...
  beforeAll(async () => {
    ...
    fs.writeFileSync(path.resolve(testFolderPath, 'firebase.json'), '', {
      flag: 'w',
    });
  ...
  it('should create example index files', () => {
    expect(fs.existsSync(path.resolve(initPath, 'src/index.html'))).toEqual(
      true
    );
    expect(fs.existsSync(path.resolve(initPath, 'src/index.js'))).toEqual(true);
    expect(fs.existsSync(path.resolve(initPath, 'src/index.css'))).toEqual(
      true
    );
  });
  ...
  it('should create environment files', () => {
    expect(fs.existsSync(path.resolve(initPath, '.env.production'))).toEqual(
      true
    );
...
describe('fliff init fail', () => {
  describe('when path already exist', () => {
    ...
    it('reject with error', async () => {
      await expect(fliff.init(cachePath)).rejects.toThrow(
        FLIFF.ErrorMessages.FailedToInitPathExists(cachePath)
      );
    });
  ...
  describe('when has unknown error while creating path', () => {
    ...
    beforeAll(() => {
      jest
        .spyOn(fs, 'mkdir')
        .mockImplementation((initPath, mkDirOptions, callback) => {
    ...
    beforeAll(() => {
      jest.mock('recursive-copy', () => {
        return jest
          .fn()
          .mockImplementation(
            (webviewPath, initPath, copyOptions, callback) => {
