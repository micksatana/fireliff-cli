  ...
  describe('getDeviceByDeviceUser', () => {
    ...
    describe('when success', () => {
      ...
      beforeAll(async () => {
        ...
        ThingsGetDeviceByDeviceUserRequest.mockImplementation(() => {
          return req;
        });
        things = new Things();
      ...
      it('create request with access token', () => {
        expect(ThingsGetDeviceByDeviceUserRequest).toHaveBeenCalledWith({
          accessToken,
        });
      });
    ...
    describe('when failed with 401', () => {
      ...
      beforeAll(async () => {
        ...
        things = new Things();
        await things
          .getDeviceByDeviceUser(deviceId, userId)
          .catch((catchedError) => (result = catchedError));
      });

      it('reject with error', () => {
        expect(result).toEqual(
          new ThingsError(Things.ErrorMessages.FailedToGetDeviceByDeviceUser401)
        );
      });
    });
    ...
    describe('when failed with 404', () => {
      ...
      it('reject with error', async () => {
        expect(result).toEqual(
          new ThingsError(Things.ErrorMessages.FailedToGetDeviceByDeviceUser404)
        );
      });
    });
    ...
    describe('when failed with unknown', () => {
      ...
      it('reject with error', async () => {
        expect(result).toEqual(
          new ThingsError(Things.ErrorMessages.FailedToGetDeviceByDeviceUser)
        );
      });
    });
  });
