  ...
  describe('getTrialProducts', () => {
    ...
    describe('when success', () => {
      ...
      beforeAll(async () => {
        ...
        ThingsGetTrialProductsRequest.mockImplementation(() => {
          return req;
        });
        things = new Things();
      ...
      it('create request with access token', () => {
        expect(ThingsGetTrialProductsRequest).toHaveBeenCalledWith({
          accessToken,
        });
      });
    ...
    describe('when failed with 401', () => {
      ...
      it('reject with error', () => {
        expect(result).toEqual(
          new ThingsError(Things.ErrorMessages.FailedToGetTrialProducts401)
        );
      });
    });
    ...
    describe('when failed with 404', () => {
      ...
      it('reject with error', async () => {
        expect(result).toEqual(
          new ThingsError(Things.ErrorMessages.FailedToGetTrialProducts404)
        );
      });
    });
    ...
    describe('when failed with unknown', () => {
      ...
      it('reject with error', async () => {
        expect(result).toEqual(
          new ThingsError(Things.ErrorMessages.FailedToGetTrialProducts)
        );
      });
    });
  });
